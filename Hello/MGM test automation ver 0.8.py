...
def summaryreport(Folder1, resulltfile1):
    ret=pd.DataFrame
    fflag=False
    sflag=False
    global Folder
    ...
    writer = pd.ExcelWriter(resulltfile + ".xls", engine=None)
    df4.to_excel(writer, sheet_name='Summary')
    ret.to_excel(writer, sheet_name='Sheet1')
    writer.save()

summaryreport("d:/report/MGM","d:/report/MGM/Result/Result2")
